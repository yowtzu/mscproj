...
config = JohansenStochasticProcessConfig(135)
target = StochasticProcess(config)
target.plot()
...
class SIR:
    ...
    def filter_dist(self, c=0.95):
        ...
        xmax = np.zeros(self.N)
        xmin = np.zeros(self.N)
        ...
        for t in range(self.N):
            ...
            for n in range(self.N):
                if sorted_cum_ws[n] < c_min:
                    min_ind = n
                if sorted_cum_ws[self.N - n] > c_max:
                    max_ind = self.N - n 
